...
private:
	...
	void copy (const basic_buffer & other) noexcept {
		base::setg(other.eback(), other.gptr(), other.egptr());
		base::setp(other.pbase(), other.epptr());
		base::pbump(other.epptr() - other.pbase());
	}
public:
	basic_buffer () = default;
	basic_buffer (const basic_buffer & other) noexcept : base() {
		copy(other);
	}
	basic_buffer & operator = (const basic_buffer & rhs) noexcept {
		copy(rhs);
		return *this;
	}
	using base::base;
